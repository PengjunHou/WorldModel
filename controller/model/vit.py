...
class PatchEmbed2(nn.Module):
    def __init__(self, embed_dim, use_norm, num_channel=3, img_h=96, img_w=96):
        ...
        H1 = conv2d_out_size(img_h, kernel_size=8, stride=4)
        W1 = conv2d_out_size(img_w, kernel_size=8, stride=4)
        H2 = conv2d_out_size(H1, kernel_size=3, stride=2)
        W2 = conv2d_out_size(W1, kernel_size=3, stride=2)
...
class MinVit(nn.Module):
    def __init__(
        ...
        # weight init
        trunc_normal_(self.pos_embed, std=0.02)     # initialize weights with Truncated Normal Distribution（截断正态分布）to avoid large deviation
