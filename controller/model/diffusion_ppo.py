...
from controller.model.diffusion_vpg import VPGDiffusion
...
if __name__ == "__main__":
    ...
    To = 6
    Ta = 4
    Do = obs_shape[0] * obs_shape[1] * obs_shape[2]
    Da = 2
    K = 8

    obs = torch.randn(B, To, Do)
    chains_prev = torch.randn(B, K + 1, Ta, Da)
    chains_next = torch.randn(B, K + 1, Ta, Da)
    denoising_inds = torch.randint(0, K, (B,))
    returns = torch.randn(B)
    values = torch.randn(B)
    advantages = torch.randn(B)
    oldlogprobs = torch.randn(B, K, Ta, Da)
    ...
    print(
        f"pg_loss {pg_loss.item():.4f} entropy_loss {entropy_loss.item():.4f} v_loss {v_loss.item():.4f} clipfrac {clipfrac:.4f} approx_kl {approx_kl:.4f} ratio {ratio:.4f} bc_loss {bc_loss.item():.4f} eta {eta:.4f}"
    )
