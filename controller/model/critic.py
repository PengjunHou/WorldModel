...
from controller.model.common import MLP, ResidualMLP, RandomShiftsAug, SpatialEmb
...
# Use This for image-based critic
class ViTCritic(CriticObs):
    ...
    def forward(
        ...
        """
        rgb_local = cond["state_local_maps"].unsqueeze(1)   
        rgb_fused = cond["state_fused_maps"].unsqueeze(1)    
        rgb = torch.cat([rgb_local, rgb_fused], dim=2).squeeze(3)
        B, T_rgb, C, H, W = cond["state_local_maps"].shape

        # flatten history
        state = cond["state_Ks"].view(B, -1)
