...
from controller.model.diffusion import DiffusionModel, Sample
from controller.model.sampling import make_timesteps, extract
...
class VPGDiffusion(DiffusionModel):
    ...
    def p_mean_var(
        ...
        # Use base policy to query expert model
        actor = self.actor if use_base_policy else self.actor_ft

        if len(ft_indices) > 0:
            cond_ft = {key: cond[key][ft_indices] for key in cond}
            #print(f"ft_indices: {ft_indices}")
            #print(f"shape of cond_ft: {{k: v.shape for k, v in cond_ft.items()}}")
            noise_ft = actor(x[ft_indices], t[ft_indices], cond=cond_ft)
            #print("noise_ft nan?", torch.isnan(noise_ft).any(), "max:", noise_ft.max().item(), "min:", noise_ft.min().item())
        ...
        if self.predict_epsilon:
            if self.use_ddim:
                alpha = extract(self.ddim_alphas, index, x.shape).clamp(min=1e-6)
                alpha_prev = extract(self.ddim_alphas_prev, index, x.shape).clamp(min=1e-6)
                sqrt_one_minus_alpha = extract(
                    self.ddim_sqrt_one_minus_alphas, index, x.shape
                ).clamp(min=1e-6)

                x_recon = (x - sqrt_one_minus_alpha * noise) / (alpha.sqrt() + 1e-6)
            else:
                sqrt_recip_alphas = extract(self.sqrt_recip_alphas_cumprod, t, x.shape).clamp(min=1e-6)
                sqrt_recipm1_alphas = extract(self.sqrt_recipm1_alphas_cumprod, t, x.shape).clamp(min=1e-6)

                x_recon = sqrt_recip_alphas * x - sqrt_recipm1_alphas * noise
        else:
            x_recon = noise
        ...
        if self.denoised_clip_value is not None:
            ...
            if self.use_ddim:
                noise = (x - alpha.sqrt() * x_recon) / (sqrt_one_minus_alpha + 1e-6)
        ...
        if self.use_ddim:
            if deterministic:
                ...
                etas = torch.sigmoid(self.eta(cond)).unsqueeze(1)  # 保证在 (0,1)

            sigma = (
                etas
                * ((1 - alpha_prev) / (1 - alpha) * (1 - alpha / alpha_prev)).clamp(min=1e-10).sqrt()
            ).clamp(min=1e-10)

            dir_xt_coef = (1.0 - alpha_prev - sigma**2).clamp(min=0).sqrt()
            mu = (alpha_prev.sqrt()) * x_recon + dir_xt_coef * noise

            var = sigma**2
            var = var.clamp(min=1e-10)
            logvar = torch.log(var + 1e-10)
        else:
            mu = (
                extract(self.ddpm_mu_coef1, t, x.shape).clamp(min=1e-6) * x_recon
                + extract(self.ddpm_mu_coef2, t, x.shape).clamp(min=1e-6) * x
            )
            logvar = extract(self.ddpm_logvar_clipped, t, x.shape).clamp(min=-30, max=20)
            etas = torch.ones_like(mu, device=mu.device)

        # Debug: 检查是否有 NaN / Inf
        # if torch.isnan(mu).any() or torch.isinf(mu).any():
            #print("⚠️ Warning: NaN/Inf detected in mu")
            #print("alpha range:", alpha.min().item(), alpha.max().item())
            #print("alpha_prev range:", alpha_prev.min().item(), alpha_prev.max().item())
    ...
    def forward(
        ...
        sample_data = cond["state_Ks"] if "state_Ks" in cond else cond["state_local_maps"]
        B = len(sample_data)
    ...
    def get_logprobs_subsample(
        ...
        #print(f"next_mean shape: {next_mean.shape}, logvar shape: {logvar.shape}")
        #print(f"next_mean: {next_mean}, logvar: {logvar}")
        std = torch.exp(0.5 * logvar)
        std = torch.clip(std, min=self.min_logprob_denoising_std)
        dist = Normal(next_mean, std)
