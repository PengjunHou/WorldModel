...
import matplotlib
from src.extract.vae_module import PerceptionMapVAE, vae_loss
...
class PerceptionMapDataset(Dataset):
    def __init__(self, env_config, model_config: Config):
        ...
        self.cache_path = os.path.join(model_config.VAE_PRETRAIN_DATA_DIR, "vae_dataset.npy")
        if os.path.exists(self.cache_path):
            ...
            for _ in range(100):
                ...
                while not done:
                    ...
                    for i in range(env_config.num_vehicles):
                        self.samples.append(np.expand_dims(states['fused_maps'][i][-1], axis = 0))  # [1, H, W]
                        num_samples +=1 
                        if num_samples >= model_config.VAE_NUM_SAMPLES:
                            np.save(self.cache_path, np.array(self.samples))
                            print(f"✓ Cached dataset saved: {self.cache_path}")
                            break
                    if num_samples >= model_config.VAE_NUM_SAMPLES:
...
def train_vae(env_config, model_config: Config):
    ...
    # ========== 2. 创建模型 ==========
    MAP_HEIGHT, MAP_WIDTH = dataset.env.dataset.local_bev_config['grid_size']
    ...
    if os.path.exists(model_config.VAE_CHECKPOINT_PATH):
        print(f"✓ Found checkpoint: {model_config.VAE_CHECKPOINT_PATH}")
        checkpoint = torch.load(model_config.VAE_CHECKPOINT_PATH, map_location=device)
    ...
    for epoch in range(start_epoch, model_config.VAE_PRETRAIN_EPOCHS):
        ...
        # ========== 5. 保存 checkpoint ==========
        os.makedirs(model_config.VAE_CHECKPOINT_DIR, exist_ok=True)
        torch.save({
            'epoch': epoch + 1,
            'model_state_dict': vae.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'loss': avg_loss
        }, model_config.VAE_CHECKPOINT_PATH)
        print(f"✓ Saved checkpoint: {model_config.VAE_CHECKPOINT_PATH}")
    ...
    save_path = os.path.join(save_dir, "vae_loss_resume.png")
    plt.savefig(save_path, dpi=200)
    print(f"✓ Loss curve saved to {save_path}")

    return vae

def train_vae2(env_config, model_config: Config ):
    """训练VAE"""
    print("\n" + "="*60)
    ...
    # 创建模型
    MAP_HEIGHT, MAP_WIDTH = dataset.env.dataset.local_bev_config['grid_size'][0], dataset.env.dataset.local_bev_config['grid_size'][1]
    vae = PerceptionMapVAE(
        map_size=(MAP_HEIGHT, MAP_WIDTH),
        latent_dim=model_config.VAE_LATENT_DIM
    ).to(device)
    ...
    loss_log = []
    vae.train()
    for epoch in range(model_config.VAE_PRETRAIN_EPOCHS):
        total_loss = 0
        num_batches = 0
        ...
        if (epoch + 1) % 10 == 0:
            ...
            checkpoint_path = os.path.join(
                model_config.VAE_CHECKPOINT_DIR,
    ...
    # 保存最终模型
    final_path = model_config.VAE_CHECKPOINT_PATH
    os.makedirs(os.path.dirname(final_path), exist_ok=True)
    torch.save({
        'epoch': model_config.VAE_PRETRAIN_EPOCHS,
        'model_state_dict': vae.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'loss': avg_loss
    }, final_path)
    ...
    plt.tight_layout()
    save_dir=model_config.VAE_VISUALIZE_DIR
    save_path = os.path.join(save_dir, "vae_loss.png")
    plt.savefig(save_path, dpi=200)
...
def load_trained_vae(env_config, model_config:Config, checkpoint_path, device="cuda"):
    ...
    MAP_HEIGHT, MAP_WIDTH = env.dataset.local_bev_config['grid_size']
    ...
    print(f"✓ Loaded pretrained VAE from {checkpoint_path} (epoch {checkpoint['epoch']})")
    return vae
...
def visualize_vae_reconstruction(vae, dataloader, device, save_dir, num_samples=4):
    ...
    with torch.no_grad():
        ...
        plt.tight_layout()
        save_path = os.path.join(save_dir, "vae_reconstruction.png")
        plt.savefig(save_path, dpi=200)
        plt.close(fig)
...
def test_single_sample():
    ...
    # ====== 2. 初始化模型与优化器 ======
    vae = PerceptionMapVAE(map_size=(16, 16), latent_dim=64, beta=0.1).to(device)
    ...
    plt.tight_layout()
    save_dir=model_config.VAE_VISUALIZE_DIR
    save_path = os.path.join(save_dir, "test_vae_reconstruction.png")
    plt.savefig(save_path, dpi=200)  
...
if __name__ == '__main__':
    ...
    vae = load_trained_vae(
        ...
        checkpoint_path=model_config.VAE_CHECKPOINT_PATH,
        device="cuda" if torch.cuda.is_available() else "cpu"
    )
