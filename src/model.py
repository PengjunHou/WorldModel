...
from src.extract.feature_extractor import ComprehensiveFeatureExtractor
from src.gnn.stgat_module import SpatioTemporalGAT, EdgeBuilder
from src.qnet.q_generator import CNNQGenerator
...
class STGATQNetwork(nn.Module):
    ...
    def forward(self, batch_data: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        for t in range(T):
            ...
            # 构建当前时刻的通信边（支持边权重）
            use_edge_weights = getattr(self.model_config, 'USE_EDGE_WEIGHTS', False)
