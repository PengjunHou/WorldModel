...
ap.add_argument(
    ...
    type=str,
    default="mnist",
    choices=["mnist", "cifar10", "celeba", "v2x_sim_mini"],
...
ap.add_argument(
    ...
    default=os.path.join(CONFIGPATH, "configs/base_training_config.json"),
)
ap.add_argument(
    "--use_wandb",
...
ap.add_argument(
    ...
    choices=["arr_train.npz", "img_train.npz", "pts_train.npz"],
    required=True,
)
ap.add_argument(
    "--eval_file",
    help="eval data file",
    choices=["arr_eval.npz", "img_eval.npz", "pts_eval.npz"],
    required=True,
)
...
def main(args):
    ...
    try:
        ...
        train_data = (
            np.load(os.path.join(DATAPATH, f"data/{args.dataset}", args.train_file))[
                "data"
            ]
            / 255.0
        )
        # train_data = (
        #     np.load(os.path.join(DATAPATH, f"data/{args.dataset}", "train_data.npz"))[
        #         "data"
        #     ]
        #     / 255.0
        # )
        eval_data = (
            np.load(os.path.join(DATAPATH, f"data/{args.dataset}", args.eval_file))["data"]
            / 255.0
        )
        # eval_data = (
        #     np.load(os.path.join(DATAPATH, f"data/{args.dataset}", "eval_data.npz"))["data"]
        #     / 255.0
        # )
