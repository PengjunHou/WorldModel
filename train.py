...
def master():
    ...
    for it in range(iters):
        ...
        total_rewards = torch.zeros((1), dtype=torch.float32, device=env.RL_device)
        while not done:
            states = env.wrapper_state(obs)   # 获取状态特征和邻接矩阵
            # states_tuple = (local_maps, fused_maps, position, interested_maps, adj)
            #print(f"current max time slices cnt {max_k.item()}, {type(max_k)}")
            score_map, action, logp, value = env.action_select(states, max_k=50)
