...
class TrajGODEModel(nn.Module):
    ...
    def forward(self, images_past, states_past, group_id, node_mask, H: int, dt: float = 1.0, k: int = 2,
                solver="dopri5", rtol=1e-3, atol=1e-4, cycle: bool = False, cycle_solver: str = "rk4"):
        """
        Returns:
        y_hat: (B,H,N,6)
        cycle_loss: scalar (only if cycle=True)
        """
        B, L, N, D = states_past.shape
        ...
        for b in range(B):
            ...
            # ===== forward integrate =====
            h_seq = odeint(self.odefunc, h0_b, t_eval, method=solver, rtol=rtol, atol=atol)  # (H,N,h_dim)
            ...
            # ===== cycle loss (optional) =====
            if cycle:
                # 从最后的 h_H 反推回 h_0（负时间积分）
                hH = h_seq[-1]  # (N,h_dim)
                t_back = torch.tensor([0.0, -float(H * dt)], device=h0_b.device, dtype=h0_b.dtype)
                h_back = odeint(self.odefunc, hH, t_back, method=cycle_solver, rtol=rtol, atol=atol)[-1]  # (N,h_dim)

                m = mask_b[:, None].float()
                cyc = ((h_back - h0_b) ** 2 * m).sum() / (m.sum() * h0_b.size(-1) + 1e-6)
                cycle_losses.append(cyc)

        pred = torch.stack(preds, dim=0)  # (B,H,N,6)

        if not cycle:
            return pred

        if len(cycle_losses) == 0:
            cycle_loss = torch.tensor(0.0, device=pred.device)
        else:
            cycle_loss = torch.stack(cycle_losses).mean()

        return pred, cycle_loss
...
def train(npz_path, epochs=10, batch_size=4, lr=3e-4, device="cuda",
    ...
    for ep in range(start_epoch, epochs + 1):
        ...
        for batch in dl_train:
            ...
            # y_hat = model(imgs, x, gid, node_mask, H=H, dt=dt, k=2, solver=solver)
            # loss = traj_loss(y_hat, y, node_mask)
            y_hat, cycle_loss = model(imgs, x, gid, node_mask, H=H, dt=dt, k=3, solver=solver, cycle=True, cycle_solver="rk4")
            loss_traj = traj_loss(y_hat, y, node_mask)

            lambda_cycle = 1  # 建议从 0.01 开始
            loss = loss_traj + lambda_cycle * cycle_loss
