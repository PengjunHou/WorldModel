    ...
    def forward(
        ...
        for t in range(T):
            ...
            if self.use_edge_weights and edge_weights_history is not None:
                # print(f"Using edge weights at time step {t}, edge_weights_history shape: {len(edge_weights_history)}")
                # print(f"Edge weights shape: {edge_weights_history[t].shape}")
...
class EdgeBuilder:
    ...
    def build_communication_edges(
        ...
        print(f"Built {edge_index.shape[1]} edges with weights using '{weight_type}' method.")
